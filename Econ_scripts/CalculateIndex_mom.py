...
def loadDataSource(source: str, isBase: bool)->pd.DataFrame:
    ...
    # thisDF.head()
    thisDF = pd.read_csv(source, encoding=thisCodec, header=None, dtype={0: "object", 1: "object", 2: "object", 3: "object", 4: "object", 5: "object", 6: "objec", 7: "object", 8: "object",
                                                                            9: "float", 10: "float"})
...
if __name__ == '__main__':
    ...
    allianceCode = 'C'
    sourcePath = r"F:\FlatData\L.POINT\phase2_2\converted\C\Outliers_removed\Monthly"
    destinationPath = r"F:\FlatData\L.POINT\phase2_2\result\C"
    ...
    try:
        ...
        while thisDate <= endDate:
            ...
            logger.info('progress:t{},t-dt:{}'.format(thisDate, baseDate))

            tFile = path.join(sourcePath, allianceCode + '_tot_master_{:%Y%m}.csv'.format(thisDate))
            t_dtFile = path.join(sourcePath, allianceCode + '_tot_master_{:%Y%m}.csv'.format(baseDate))
            ...
            if path.exists(tFile) and path.exists(t_dtFile):
                ...
                with codecs.open(path.join(destinationPath, 'level1_{:%Y%m}.csv'.format(thisDate)), 'w', thisCodec) as writer:
                ...
                with codecs.open(path.join(destinationPath, 'level2_{:%Y%m}.csv'.format(thisDate)), 'w', thisCodec) as writer:
                ...
                with codecs.open(path.join(destinationPath, 'level3_{:%Y%m}.csv'.format(thisDate)), 'w', thisCodec) as writer:
